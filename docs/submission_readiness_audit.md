...
pages before References against the 9-page NeurIPS budget. The same audit now
marks venue submission ready because the public release archive, public source
repository, external CI, and local GPU-container receipts are filled; it keeps
the independent external GPU-host receipt as a warning and strict external
hardening item rather than a venue-packaging blocker. Local compute-resource,
asset-license, and new-asset metadata are documented and checked by the audit.
...
converts these local facts into the four public receipts: archive upload,
source repository, external CI, and external GPU-container validation. The
first three are observed in the receipt registry; the external GPU-container
...
theorem about all Bayesian neural-network posteriors. It is upload-ready under
the venue packaging audit because the public release archive, public source
repository, external CI, and local GPU-container receipts are filled. It is not
strict-external-validation ready because the independent external GPU-host
receipt is still missing, and the bounded full-network exact dense
covariance/permutation robustness limitations remain explicit scientific
limitations.
`docs/external_validation_receipts.json` now records the observed public
archive, public source, and external CI receipts plus the still-pending
external GPU-container receipt, while
...
plus source-only repository snapshot from the still-missing external GPU-host
evidence.
