...
| Paper draft | A coherent, compilable manuscript with tables/figures and honest limitations. | `paper/main.tex`, `paper/refs.bib`, `paper/neurips_2026.sty`, `paper/neurips_checklist.tex`, `paper/tables/statistical_summary.tex`, appendix-inclusive `paper/main.pdf`, main-only `paper/main_submission.pdf`, official-style `paper/neurips_submission.pdf`, `docs/paper_submission_shape_audit.md`, `docs/submission_pdf_shape_audit.md`, `docs/venue_submission_compliance_audit.md`, `docs/submission_handoff.md`, `docs/release_anonymization_audit.md`, `docs/public_release_archive_audit.md`; latest build passed. The abstract is under the 250-word budget, NeurIPS main content is 8 pages before References against the 9-page budget, the paper distinguishes the support-equivalence claim from general Bayesian pruning and scopes the limitation to tested posterior families, and the verifier checks cited-key coverage plus corrected key citation metadata. The submission handoff records the exact upload files, abstract/title metadata, current release archive SHA256, source snapshot commit, local check commands, and remaining release blockers. | Substantial condensed NeurIPS-bound draft; local content/PDF/style/anonymized-release/archive/source-snapshot/handoff gates are ready, final public upload and external validation remain open. |
...
| Verification | Build/test commands must cover the changed artifacts and paper. | Latest checks: `py_compile`, `make paper-check`, `make paper-submission-check`, `make check`, clean-state `scripts/verify_research_artifacts.py`, `make container-build && make container-check`, and `scripts/check_gpu_training_environment.py`; the configured GitHub Actions workflow now runs `make ci-check paper-check PYTHON=python`, local PDFs remain built, and the release manifest/anonymization/archive/extracted-package smoke, public-repository snapshot, external-validation readiness, external-validation runbook, and submission-handoff audits are regenerated by `make check`. | Good local/container/configured-CI coverage; not an externally observed CI. |
...
| "top conf" | `paper/main.tex`, `paper/main.pdf`, `paper/main_submission.pdf`, `paper/neurips_submission.pdf`, `paper/neurips_2026.sty`, `paper/neurips_checklist.tex`, `docs/submission_readiness_audit.md`, `docs/negative_result_paper_plan.md`, `docs/paper_submission_shape_audit.md`, `docs/submission_pdf_shape_audit.md`, `docs/venue_submission_compliance_audit.md`, `docs/submission_handoff.md`, `docs/release_anonymization_audit.md`, `docs/public_release_archive_audit.md`, `docs/public_repository_snapshot_audit.md`, `docs/external_validation_readiness_audit.md`, `docs/external_validation_receipt_template.md`, `docs/external_validation_runbook.md`. | Draft exists and local shape/main-only/NeurIPS PDF/content-packet/style/anonymization/archive/repository-snapshot/handoff/template/runbook audits are ready, but public upload, external CI/GPU validation, and bounded robustness gaps remain. |
...
| Build command | Latest LaTeX build completed after NeurIPS 2026 style/checklist binding; `paper/main.pdf`, `paper/main_submission.pdf`, and `paper/neurips_submission.pdf` exist, and `make paper-check` covers all three. | Covered for current draft. |
| Test command | `make check`, clean-state `scripts/verify_research_artifacts.py`, `make container-build && make container-check`, and `scripts/check_gpu_training_environment.py` passed after the latest changes; the configured CI command is now `make ci-check paper-check PYTHON=python`, and `make check` includes the release-anonymization, release-archive, extracted-package smoke, public-repository snapshot, external-validation readiness, external-validation runbook, and submission-handoff audits. | Good local/container/configured-CI coverage; not an externally observed CI run. |
...
4. Reproducibility hardening: `requirements.txt`, `requirements-ci.txt`,
   `requirements-lock.txt`, `Dockerfile`, `Dockerfile.gpu`, `.dockerignore`,
   ...
   public receipts needed before claiming release readiness. An externally
   observed green CI run, external GPU-container validation, public source
   repository receipt, and public archive-upload receipt are still missing.
...
Do not mark the thread goal complete. The next concrete milestone is external
validation: publish the anonymous source snapshot and release archive, collect
the public repository, archive, CI, and external GPU-container receipts in
`docs/external_validation_receipts.json`, then rerun the strict external gate.
If that is blocked, the remaining productive research work is a qualitatively
different posterior/permutation robustness check beyond the current full-weight
joint-group direct row, followed by paper and audit integration.
