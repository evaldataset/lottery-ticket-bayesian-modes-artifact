...
def external_release_packaging_flags(path: Path) -> list[str]:
    fallback = [
...
def build_audit(
    ...
    release_packaging_risk_flags = external_release_packaging_flags(
    ...
    if release_packaging_risk_flags:
        if release_packaging_risk_flags == ["gpu_container_run_not_observed"]:
            blocking_next_steps.append(
