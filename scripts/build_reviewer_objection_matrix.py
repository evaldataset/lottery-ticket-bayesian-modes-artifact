...
def build_rows(
    ...
    add_objection(
        ...
        current_answer=(
            "The current evidence spans low-rank Hessian-plus-diagonal, exact "
            "block/joint-group covariance, a 270,896-weight direct joint-group "
            "row, and exact dense small-model code-path checks; literal dense "
            "CIFAR covariance is outside the workstation budget."
        ),
        key_numbers=(
    ...
    add_objection(
        rows,
        objection="The artifact is not yet a submit-ready package.",
        reviewer_risk="Even strong evidence can fail review if reproducibility and paper packaging are weak.",
        current_answer=(
            "Local checks, main-only submission PDF, clean artifact verification "
            "after `make clean`, and the CPU container path pass. The venue "
            "compliance audit now binds the draft to the official NeurIPS 2026 "
            "style/checklist and marks the main content plus venue binding "
            "ready. Local compute-resource, asset-license, and new-asset "
            "metadata plus a top-level MIT license are verifier-checked, and "
            "the local anonymous-review release tarball is built and "
            "archive-audited, and an extraction smoke test verifies the package "
            "hashes plus the release-package-mode verifier. External upload, "
            "repository, CI, and GPU-host receipts are now tracked by a "
            "separate readiness audit, and a source-only anonymous git snapshot "
            "is staged locally for public-repository upload. "
            "Final upload readiness remains open because the public archive, "
            "public source repository, external CI, and GPU-host validation "
            "receipts are not complete."
        ),
        key_numbers="latest local `make check`, `make paper-check`, `make paper-submission-check`, clean verifier, and `make container-check` passed in this workspace; main-only PDF is 9 pages; NeurIPS audit enforces a 9-page main-content budget before references; abstract is within the 250-word budget; release archive audit, extraction smoke, and public-repository snapshot report ready; external-validation strict gate remains pending external receipts.",
        evidence=[
        ...
        status="Open packaging limitation",
        remaining_gap="Public release upload, clean public repository state, external CI, and GPU-container receipts are still required.",
    )
...
def write_markdown(rows: list[dict[str, Any]], path: Path) -> None:
    lines = [
        ...
        "claim that the project is submission-ready; it is a compact review",
        "risk register for the current negative-result paper.",
        "",
