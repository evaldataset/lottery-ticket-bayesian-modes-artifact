...
def require_venue_submission_compliance_audit() -> None:
    ...
    if payload.get("checklist_release_ready") is not False:
        fail("venue submission audit should track unresolved checklist release risks")
    if payload.get("release_packaging_ready") is not False:
        fail("venue submission audit should track unresolved external packaging risks")
    if payload.get("venue_submission_ready") is not False:
        fail("venue submission audit should not claim final upload readiness")
    ...
    if checklist_flags != {"public_code_or_data_not_yet_open"}:
    ...
    for flag in [
        ...
        if flag not in packaging_flags:
            fail(f"venue submission audit missing release packaging flag: {flag}")
    text = " ".join(doc_path.read_text(encoding="utf-8").split())
    for phrase in [
        "Venue Submission Compliance Audit",
        "Current content-packet status: ready",
        "Current venue-binding status: ready",
        "Current checklist-release status: not ready",
        "Current release-packaging status: not ready",
        ...
        "public_code_or_data_not_yet_open",
        "external_ci_run_not_observed",
    ]:
        if phrase not in text:
...
def require_public_repository_snapshot_audit() -> None:
    ...
    for phrase in [
        ...
        "full run-artifact package remains the separate public",
        "Git commit",
        "Risk Flags",
        "- none",
...
def require_external_validation_readiness_audit() -> None:
    ...
    if payload.get("external_validation_ready") is True:
        ...
        for flag in [
            ...
            if flag not in risk_flags:
                fail(f"external validation audit missing expected open external flag: {flag}")
    ...
    if payload.get("external_validation_ready") is not True:
        required_phrases.extend(
            ["public_release_upload_not_verified", "external_ci_run_not_observed"]
        )
